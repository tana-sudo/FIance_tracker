  ...
  const downloadFullPDF = async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(reportRef.current, { 
        ...
        useCORS: true // Handles external images/icons
      });
      ...
      // 3. Calculate the total height the image will have in the PDF
      const imgRatio = imgHeight / imgWidth;
      const totalPdfHeight = pdfPageWidth * imgRatio;

      let heightLeft = totalPdfHeight;
      let position = 0; // This will be the Y-offset for the image

      // 4. Add the first page (or the only page, if it fits)
      pdf.addImage(imgData, "PNG", 0, position, pdfPageWidth, totalPdfHeight);
      heightLeft -= pdfPageHeight; // Subtract the height of one page

      // 5. Loop and add new pages as long as there's content left
      while (heightLeft > 0) {
        position = -heightLeft; // Shift the image "up" by the height of one page
        pdf.addPage();
        // Add the same image, but at the new Y-offset
        pdf.addImage(imgData, "PNG", 0, position, pdfPageWidth, totalPdfHeight);
        heightLeft -= pdfPageHeight; // Subtract another page height
      }
